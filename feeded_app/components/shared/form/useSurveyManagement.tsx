...
export const useSurveyManagement = () => {
      ...
      const programList = await Promise.all(
        programSnapshot.docs.map(async (doc) => {
          ...
          const studentsCollection = collection(
            db,
            `users/${user.uid}/programs/${doc.id}/students`
          );
          ...
          return {
            ...
            ...programData,
            students,
            reponsesChaud: programData.reponsesChaud || {
              envoye: 0,
              enAttente: 0,
              relance: 0,
            },
            reponsesFroid: programData.reponsesFroid || {
              envoye: 0,
              enAttente: 0,
              relance: 0,
            },
  ...
  const sendSurveyEmail = useCallback(
    async (
      ...
      const surveyLink = `http://localhost:3000/${
        type === "hot" ? "chaud" : "froid"
      ...
      try {
        ...
        console.log(
          `Email sent to ${studentData.email} for program ${programName} (${type})`
        );
  ...
  const updateSurveyCounts = useCallback(
    async (
      programId: string,
      type: "hot" | "cold",
      field: "envoye" | "enAttente" | "relance"
    ) => {
      const user = auth.currentUser;
      if (!user) {
      ...
      try {
        await updateDoc(programDocRef, {
          [`reponses${type === "hot" ? "Chaud" : "Froid"}.${field}`]:
            increment(1),
        });
        console.log(
          `Updated ${type} survey ${field} count for program ${programId}`
        );
      } catch (error) {
        console.error(
          `Error updating ${type} survey ${field} count for program ${programId}:`,
          error
        );
      }
    },
  ...
  const updateStudentEmailStatus = useCallback(
    async (programId: string, studentId: string, type: "hot" | "cold") => {
      const user = auth.currentUser;
      if (!user) {
        console.error("No authenticated user found");
        return;
      ...
      try {
        ...
        console.error(
          `Error updating ${type} email status for student ${studentId}:`,
          error
        );
      }
    },
  ...
  const sendHotSurveys = useCallback(
    async (programId: string) => {
      ...
      for (const student of updatedProgram.students) {
        if (!student.hotEmailSent) {
          try {
            ...
            await updateStudentEmailStatus(
              updatedProgram.id,
              student.id,
              "hot"
            );
            ...
            console.error(
              `Failed to send hot survey email to ${student.email}:`,
              error
            );
      ...
      if (sentCount > 0) {
        showAlert(
          "success",
          `Successfully sent ${sentCount} hot survey${
            sentCount > 1 ? "s" : ""
          }.`
        );
      }
      if (errorCount > 0) {
        showAlert(
          "error",
          `Failed to send ${errorCount} hot survey${errorCount > 1 ? "s" : ""}.`
        );
      }
  ...
  const sendColdSurveys = useCallback(
    async (programId: string) => {
      const program = programs.find((p) => p.id === programId);
      if (!program) {
        console.error(`Program with id ${programId} not found`);
      ...
      for (const student of updatedProgram.students) {
        if (!student.coldEmailSent) {
          try {
            ...
            console.error(
              `Failed to send cold survey email to ${student.email}:`,
              error
            );
      ...
      if (sentCount > 0) {
        showAlert(
          "success",
          `Successfully sent ${sentCount} cold survey${
            sentCount > 1 ? "s" : ""
          }.`
        );
      }
      if (errorCount > 0) {
        showAlert(
          "error",
          `Failed to send ${errorCount} cold survey${
            errorCount > 1 ? "s" : ""
          }.`
        );
      }
  ...
  const fetchFormAnswersByProgram = useCallback(
    async (programId: string): Promise<OrganizedFormAnswers> => {
      console.log(
        `Starting fetchFormAnswersByProgram for programId: ${programId}`
      );
      const user = auth.currentUser;
      if (!user) {
        console.error("No authenticated user found");
      ...
      try {
        ...
        console.log(
          `Organized answers:`,
          JSON.stringify(organizedAnswers, null, 2)
        );
        return organizedAnswers;
      } catch (error) {
        console.error("Error fetching form answers for program:", error);
        throw new Error(
          "Unable to fetch form answers for the specified program."
        );
      }
    },
    []
  ...
  const generateCSV = useCallback(
    (formAnswers: FormAnswer[], formType: "hot" | "cold"): string => {
      ...
      if (formType === "hot") {
        ...
        for (const answer of formAnswers) {
          ...
          const row = [
            ...
            escapeCSVField(
              hotAnswers.applicationConnaissances?.toString() ?? ""
            ),
        ...
        headers = [
          ...
          "Submitted At",
          "Confiance",
          "Emploi Grace Formation",
          "Formation",
          "Impact Situation",
          "Mise En Pratique",
          "Nom",
          "Pertinence Connaissances",
          "Prenom",
          "Recommandation",
          "Reponse Besoins",
          "Situation Professionnelle",
          "Situation Professionnelle Autre",
          "Suggestions",
        ];
        csvRows = [headers.join(",")];
        ...
        for (const answer of formAnswers) {
          ...
          const row = [
            ...
            escapeCSVField(
              coldAnswers.pertinenceConnaissances?.toString() ?? ""
            ),
  ...
  const downloadFormAnswers = useCallback(
    (
      formType: "hot" | "cold",
      currentFormAnswers: OrganizedFormAnswers | null = formAnswers
    ) => {
      console.log(
        "Attempting to download form answers. Current formAnswers state:",
        currentFormAnswers
      );
      ...
      if (!currentFormAnswers) {
        ...
        showAlert(
          "error",
          "No form answers available. Please fetch the data first."
        );
        return;
      }

      const answers =
        formType === "hot" ? currentFormAnswers.hot : currentFormAnswers.cold;
